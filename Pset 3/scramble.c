...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        for (int row = 0; row < DIMENSION; row++)
        {
            ...
            {
                fprintf(logfile, "%c", grid[row][column]);
                fprintf(logfile, "\n");
            }
        }
...
void draw(void)
{
    ...
    for(int row = 0; row < DIMENSION; row++)
    {
        // Print letter in each column with space
        for(int column = 0; column < DIMENSION; column++)
        {
...
bool lookup(string s)
{
    ...
    // Iterate until start surpasses end
    while (start <= end)
    {
        // Update middle
        int middle = (start + end) / 2;
        
        // If string is in the middle
        if (strcmp(s, dictionary.words[middle].letters) == 0)
        {
            // If the word has previously been found, return false
            if (dictionary.words[middle].found)
            {
                return false;
            }
            // If the word hasn't been found previously, return true;
            else
            {
        ...
        // S is greater than the word -> shift start to pass middle
        if (strcmp(s, dictionary.words[middle].letters) > 0)
        {
            start = middle + 1;
        }
        // S is less than the word -> shift end to pass middle
        else
        {
            end = middle - 1;
        }
...
void scramble(void)
{
    ...
    for(int row = 0; row < DIMENSION; row++)
    {
        // Save current grid as temp grid
        for(int column = 0; column < DIMENSION; column++)
        {
    ...
    // Shift every character over one
    for(int i = 0; i < DIMENSION; i++)
    {
        for(int j = (DIMENSION - 1); j >= 0; j--)
        {
            grid[i][(DIMENSION - 1) - j] = tempGrid[j][i];
        }
    }
